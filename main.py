#!/usr/bin/env python3
"""
RSS Intelligence Pipeline - v3.1 (Hotfix)
...
def load_cache(date_str: str) -> List[Dict]:
    ...
    try:
        ...
        if not isinstance(data, list):
            ...
            return []
        if len(data) == 0:
            print(f"  ⚠️  Cache empty — ignoring: {cp}")
            return []
...
class DatabaseManager:
        ...
        try:
            ...
            for r in res:
                ...
                if isinstance(d, datetime):
                    ...
                    d = str(d)
                out.append({'date': d, 'count': r[1], 'normalized': r[2]})
            return out
        except Exception:
            return []
    ...
    def archive_old_data(self):
        ...
        with gzip.open(af, 'wt', encoding='utf-8') as f:
            json.dump(articles, f, indent=2, default=json_default)
...
class KeywordExtractor:
    STOPWORDS = {
        ...
        # HTML / URL noise
        'span', 'class', 'href', 'https', 'http', 'www', 'com', 'org', 'net',
        'html', 'div', 'style', 'strong', 'target', 'blank', 'rel', 'noopener',
        'noreferrer', 'src', 'alt', 'title', 'id', 'name', 'type', 'value',
        'script', 'nbsp', 'amp', 'gt', 'lt', 'quot', 'apos', 'ndash', 'mdash',
    }
...
class WordStatsAnalyzer:
    ...
    def analyze(self, articles: List[Dict]) -> Dict[str, Any]:
        ...
        for word, count in word_counts.items():
            ...
            if base > 0:
                avg_base = base / 7.0
                std = max(1.0, avg_base * 0.5)
                z = (count - avg_base) / std
            else:
                avg_base = 0.0
                std = 1.0
                z = 0.0
        ...
        spikes = {w: s for w, s in word_stats.items() if s['baseline'] > 0 and s['z_score'] > Z_SCORE_THRESHOLD}
        drops = {w: s for w, s in word_stats.items() if s['baseline'] > 0 and s['z_score'] < -Z_SCORE_THRESHOLD}
...
def main():
    ...
    print("\n" + "=" * 50)
    print("  RSS Intelligence Pipeline v3.1")
    print("=" * 50 + "\n")
    ...
    if not all_articles:
        ...
        for idx, feed in enumerate(feeds, 1):
            ...
            for entry in result['articles']:
                ...
                clean_summary = KeywordExtractor._strip_html(raw_summary)
                text = clean_summary
                if not text or len(text.strip()) < 20:
                    continue
    ...
    report = rg.generate(all_articles, feed_stats, word_stats,
                         total_feeds=len(feeds), from_cache=from_cache)
